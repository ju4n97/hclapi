...
Query a single resource, verify `rows_affected`, and return a standard `problem()` payload.
...
Two inserts execute atomically inside a transaction, with constraint collisions converted to `409 Conflict`.
...
Multiple independent queries execute concurrently and are joined into one response payload.
