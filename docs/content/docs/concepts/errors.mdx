...
Native `go` steps can return a `hclapi.ProblemDetailsError` to halt the pipeline and emit a specific HTTP status code, type URN, and custom error details directly to the client:
...
engine.RegisterStep("auth.verify_key", func(ctx context.Context, step *hclapi.Step) (any, error) {
    ...
    if apiKey == "" {
        return nil, hclapi.ProblemDetailsError{
...
When embedding hclapi into Go, you can replace the default response shape with a custom `ErrorHandler`. See [Go integration](../../guides/go).
:::
