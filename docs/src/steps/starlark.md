...
Executes sandboxed Starlark for data transformation, filtering, and
business logic.
...
The script must define `execute(ctx)`, returning a dict, list, or scalar.
`ctx` mirrors the [execution context](../concepts/context.md), using
Starlark's dict access instead of dotted attributes.
...
Starlark scripts have no file system or network access. Iteration order and
arithmetic are deterministic across platforms. Unbounded recursion and
infinite loops are prevented by the runtime.
