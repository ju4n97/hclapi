...
func TestExecute(t *testing.T) {
	...
	t.Run("Successful execution", func(t *testing.T) {
		...
		handler := func(ctx context.Context, step *runtime.Step) (any, error) {
			name := step.Args.GetOr("name", "")
			return strings.ToUpper(name), nil
		}

		res, err := xgo.Execute(t.Context(), handler, &runtime.Step{Args: map[string]any{"name": "hello"}})
		if err != nil {
	...
	t.Run("Standard error propagation", func(t *testing.T) {
		...
		_, err := xgo.Execute(t.Context(), handler, &runtime.Step{})
