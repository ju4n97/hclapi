...
`GET /api/v1/health` responds unconditionally. `POST /api/v1/transform` runs a [`starlark`](./steps/starlark.mdx) step and returns its result. See [Pipelines and steps](./concepts/pipelines.mdx) for how steps pass data to each other.
...
The server binds to `127.0.0.1:8080`, as declared in the `server` block. Host and port can be overridden without editing the manifest; see [CLI reference](../cli/hclapi.mdx).
...
This is the standard error format used by hclapi. See [Errors](./concepts/errors.md).
