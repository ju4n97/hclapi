---
title: Quickstart
description: Create a manifest, start hclapi, and call two working endpoints.
---
...
1. Create a manifest
   ...
   `GET /api/v1/health` responds unconditionally. `POST /api/v1/transform` runs a [`starlark`](./steps/starlark.md) step and returns its result. See [Pipelines and steps](./concepts/pipelines.md) for how steps pass data to each other.
...
2. Start the server
   ...
   The server binds to `127.0.0.1:8080`, as declared in the `server` block. Host and port can be overridden without editing the manifest; see [CLI reference](./cli.md).
