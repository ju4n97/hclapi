...
| `type`     | `string` | no       | Custom type URI or slug (defaults to URN or `problem.type_prefix`)  |
...
2. **Type URI:** If `type` is omitted, the title is slugified and appended to the configured URI scheme:
   ...
   - **If `problem.type_prefix` is set in `problem {}`:** `problem.type_prefix + "<slug>"` (e.g. `"https://docs.example.com/errors/not-found"`).
